...
HPCToolkit uses instruction-level measurements to reconstruct approximate calling context trees and
...
Figure~\ref{fig:hpctoolkit}(a) shows a calling context for PeleC, an ECP application for adaptive-mesh compressible hydrodynamics code for reacting flows; the highlighted region shows an approximate reconstruction of a GPU calling context tree with multiple inlined GPU device functions.
