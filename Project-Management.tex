\section{ECP Software Technology Planning, Execution, Tracking and Assessment}
...
\begin{table}[h!]
	...
	\end{tabular}
	\caption{\label{table:KPP-3-scoring} Integration Goal Scoring: A point is accrued when a client integrates and sustainably uses a product capabilities.  Scores are assessed annually.}
\end{table}
...
\begin{table}[h!]
	...
	\end{tabular}
	\caption{\label{table:KPP-3-values} Each integration score will have an associated weight depending on the potential impact if integration targets are not met.}
\end{table}
